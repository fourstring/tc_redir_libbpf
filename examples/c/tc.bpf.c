...
static inline __u32 do_dnat(struct __sk_buff *ctx) {
	...
	struct iphdr *l3;
	void *snat_map;
	__u32 l4_csum_offset = 0, l4_id_offset, orig_ifindex;
	...
	// Csum for layer 4
	ret = bpf_l4_csum_replace(ctx, l4_csum_offset, dynamic_query_id, value->l4_id, sizeof(__u32) | IS_PSEUDO);
	if (ret < 0) {
	...
	bpf_skb_store_bytes(ctx, ICMP_ID_OFF, &value->l4_id, sizeof(__u16), 0);
...
static inline int do_snat(struct __sk_buff *ctx) {
	...
	__u32 l4_csum_offset = 0, l4_id_offset;
	__u16 l4_id = 0, dynamic_query_id, *next_id;
	int ret;
	...
	ret = bpf_l4_csum_replace(ctx, l4_csum_offset, l4_id, dynamic_query_id, sizeof(__u32) | IS_PSEUDO);
	if (ret < 0) {
		bpf_printk("bpf_l4_csum_replace failed: %d", ret);
		return TC_ACT_OK;
	}
	...
	bpf_skb_store_bytes(ctx, l4_id_offset, &dynamic_query_id, sizeof(__u16), 0);
