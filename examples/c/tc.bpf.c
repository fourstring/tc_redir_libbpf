...
static inline __u32 do_dnat(struct __sk_buff *ctx) {
	...
	__u32 *orig_ifindex_ptr = bpf_map_lookup_elem(&netns_route_map, &value->source_addr);
...
int redirect_ingress(struct __sk_buff *ctx)
{
	...
	dest_ipaddr = l3->daddr;
	__u32 *target_ifindex = bpf_map_lookup_elem(&netns_route_map, &dest_ipaddr);

	if (target_ifindex) {
		return redirect_netns(*target_ifindex);
	}
